...
const Suggestions = () => {
  ...
  const hasSuggestions = Boolean(
    resumeProcessorResponse?.suggestionsSet?.length
  );

  if (!isBackendProcessing && !hasSuggestions && !processingError)
    return null;

  function renderSuggestions() {
    const suggestionsSet = resumeProcessorResponse?.suggestionsSet ?? [];

    if (!suggestionsSet.length) return null;

...
const VectorScore = () => {
  const { isBackendProcessing, resumeProcessorResponse, processingError } =
    useGlobalStore();

  const componentRef = useRef<ElementRef<"section">>(null);

  const hasVectorScores = Boolean(
    resumeProcessorResponse?.vectorScoresSet?.length
  );
  ...
  useEffect(() => {
    if (
      ...
      !isBackendProcessing &&
      (hasVectorScores || processingError)
    ) {
      componentRef.current.scrollIntoView({ behavior: "smooth" });
    }
  }, [hasVectorScores, isBackendProcessing, processingError]);

  if (!isBackendProcessing && !hasVectorScores && !processingError)
    return null;

  function renderVectorScores() {
    const vectorScoresSet = resumeProcessorResponse?.vectorScoresSet ?? [];

    if (!vectorScoresSet.length) {
      return null;
    }

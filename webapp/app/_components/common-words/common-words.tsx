...
const CommonWords = () => {
  ...
  const hasCommonWords = Boolean(
    resumeProcessorResponse?.commonWordsSet?.length
  );

  if (!isBackendProcessing && !hasCommonWords && !processingError)
    return null;

  function renderCommonWords() {
    const commonWordsSet = resumeProcessorResponse?.commonWordsSet ?? [];

    if (!commonWordsSet.length) return null;
